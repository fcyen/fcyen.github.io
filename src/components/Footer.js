...
import { LinkedInIcon, GitHubIcon, BehanceIcon } from "./SocialIcons";
...
function SocialLink({ icon: Icon, label, ...props }) {
  return (
    <Link
      className="col-span-2 flex items-center justify-center gap-2.5 rounded-full border border-slate-600/90 py-2.5 text-sm text-slate-50 duration-200 ease-in-out hover:bg-slate-800 hover:text-white lg:gap-2 xl:gap-2.5"
      {...props}
    >
...
export function Footer({ newsletter = false }) {
  return (
    <section
      className={clsx(
        newsletter && "fixed bottom-0 left-0 w-full pt-12 sm:pt-16"
      )}
    >
        ...
        <Container>
          ...
          <div className="mx-auto grid max-w-xl items-center gap-5 lg:mx-0 lg:max-w-none lg:grid-cols-12 lg:gap-12 xl:gap-20">
            <div className="flex flex-col items-center lg:col-span-5 lg:items-start">
              <div className="grid w-full max-w-sm grid-cols-6 items-center justify-center gap-3.5 sm:max-w-none sm:grid-cols-6 lg:gap-2.5 xl:gap-3.5">
